module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.bulkInsert('Books', [{
      guestFirstName: 'Дмитрий',
      guestLastName: 'Дмитриев',
      guestPatronymic: 'Дмитриевич',
      checkInDate: '2021-12-12',
      checkOutDate: '2021-12-15',
      categoryRoom: 'Комфорт',
      guestsNumber: '2',
      email: 'dimon@ya.ru',
      telephone: '+79825091864',
      status: 'Ожидает подтверждения',
      RoomId: '3',
      comment: 'Пусто',
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      guestFirstName: 'Дмитрий',
      guestLastName: 'Дмитриев',
      guestPatronymic: 'Дмитриевич',
      email: 'dimon@ya.ru',
      checkInDate: '2021-11-15',
      checkOutDate: '2021-12-20',
      categoryRoom: 'Эконом',
      guestsNumber: '3',
      telephone: '+79825121864',
      status: 'Проживает',
      RoomId: '1',
      comment: 'Душный тип',
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      guestFirstName: 'Дмитрий',
      guestLastName: 'Дмитриев',
      guestPatronymic: 'Дмитриевич',
      email: 'dimon@ya.ru',
      checkInDate: '2021-11-20',
      checkOutDate: '2021-12-21',
      categoryRoom: 'Люкс',
      guestsNumber: '2',
      telephone: '+79825091864',
      status: 'Проживает',
      RoomId: '5',
      comment: 'Нормальный тип',
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      guestFirstName: 'Дмитрий',
      guestLastName: 'Дмитриев',
      guestPatronymic: 'Дмитриевич',
      email: 'dimon@ya.ru',
      checkInDate: '2021-11-23',
      checkOutDate: '2021-11-26',
      categoryRoom: 'Люкс',
      guestsNumber: '1',
      telephone: '+79825093456',
      status: 'Подтверждено',
      RoomId: '6',
      comment: 'Предоплата 50%',
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      guestFirstName: 'Дмитрий',
      guestLastName: 'Дмитриев',
      guestPatronymic: 'Дмитриевич',
      email: 'dimon@ya.ru',
      checkInDate: '2021-11-22',
      checkOutDate: '2021-11-25',
      categoryRoom: 'Комфорт',
      guestsNumber: '1',
      telephone: '+79825093456',
      status: 'Подтверждено',
      RoomId: '6',
      comment: 'Предоплата 50%',
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      guestFirstName: 'Дмитрий',
      guestLastName: 'Дмитриев',
      guestPatronymic: 'Дмитриевич',
      email: 'dimon@ya.ru',
      checkInDate: '2021-11-25',
      checkOutDate: '2021-11-28',
      categoryRoom: 'Люкс',
      guestsNumber: '1',
      telephone: '+79825093456',
      status: 'Подтверждено',
      RoomId: '6',
      comment: 'Предоплата 50%',
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      guestFirstName: 'Дмитрий',
      guestLastName: 'Дмитриев',
      guestPatronymic: 'Дмитриевич',
      email: 'dimon@ya.ru',
      checkInDate: '2021-12-01',
      checkOutDate: '2021-12-04',
      categoryRoom: 'Эконом',
      guestsNumber: '1',
      telephone: '+79825093456',
      status: 'Подтверждено',
      RoomId: '6',
      comment: 'Предоплата 50%',
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      guestFirstName: 'Дмитрий',
      guestLastName: 'Дмитриев',
      guestPatronymic: 'Дмитриевич',
      email: 'dimon@ya.ru',
      checkInDate: '2021-12-15',
      checkOutDate: '2021-12-18',
      categoryRoom: 'Комфорт',
      guestsNumber: '1',
      telephone: '+79825093456',
      status: 'Подтверждено',
      RoomId: '6',
      comment: 'Предоплата 50%',
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      guestFirstName: 'Дмитрий',
      guestLastName: 'Дмитриев',
      guestPatronymic: 'Дмитриевич',
      email: 'dimon@ya.ru',
      checkInDate: '2021-11-20',
      checkOutDate: '2021-11-25',
      categoryRoom: 'Люкс',
      guestsNumber: '1',
      telephone: '+79825093456',
      status: 'Подтверждено',
      RoomId: '6',
      comment: 'Предоплата 50%',
      createdAt: new Date(),
      updatedAt: new Date(),
    },
    {
      guestFirstName: 'Дмитрий',
      guestLastName: 'Дмитриев',
      guestPatronymic: 'Дмитриевич',
      email: 'dimon@ya.ru',
      checkInDate: '2021-11-19',
      checkOutDate: '2021-11-15',
      categoryRoom: 'Люкс',
      guestsNumber: '1',
      telephone: '+79825093456',
      status: 'Подтверждено',
      RoomId: '6',
      comment: 'Предоплата 50%',
      createdAt: new Date(),
      updatedAt: new Date(),
    }], {});
  },

  down: async (queryInterface, Sequelize) => {

  },
};
